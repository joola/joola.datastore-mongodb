    ...
    if (!metric.formula && metric.collection) {
      ...
      if (colQuery.type == 'plain') {
        if (plan.colQueries[colQuery.key]) {
          if (_$unwind)
            _$group = self.common.extend({}, plan.colQueries[colQuery.key].query[3].$group);
          else
...
MongoDBProvider.prototype.stats = function (collectionName, callback) {
  ...
  var self = this;
  return self.db.collection(collectionName, function (err, collection) {
    if (err)
      return callback(err);
...
MongoDBProvider.prototype.drop = function (collectionName, callback) {
  ...
  return self.db.dropCollection(collectionName, callback);
};
